    ...
    <header class="header">
      ...
      <div id="main-nav" class="collapse navbar-collapse">
        <ul class="nav navbar-nav" id="mainNav">
          ...
          <li class="active"><a href="bloodBank.php" class="scroll-link">Blood Bank</a></li>
        </ul>
      </div>
      <!--/.navbar-collapse--> 
    </nav>
          ...
          <div class="panel panel-default">
                ...
                <?php
                    ...
                    <select  class="col-md-5 col-xs-8" id="hospitalBook" style="width: 100%; margin-bottom: 25px; height: 50px; font-size: 22px; font-weight: bold; background-color: rgba(0,0,0,0.8); color: white;">
                      <option value="">Click Here Please To Choose Your Hospital :</option>
                      <?php 
     ...
     $.ajax({
        ...
        beforeSend: function() {
         ...
         success: function(data){
            var html = null;
            if (data == null) {
              $('#tableBody').html('<div class="alert alert-danger">Unfortunately There Is No Blood in this hospital</div>');
            } else {
              for (x in data) {
                html += `
                <tr>
